...
export function parseBudgetData({ budget, date, duration }) {
  ...
  budget.categories.map((category) => {
    ...
    months.map((month) => {
      ...
      category.budgetedValueRange.map((range) => {
        ...
        if (
          range.effectiveEndDate === null &&
          month.date >= dateAddTZ(new Date(range.effectiveStartDate))
        ) {
        ...
        if (
          month.date < dateAddTZ(new Date(range.effectiveEndDate)) &&
          month.date >= dateAddTZ(new Date(range.effectiveStartDate))
        ) {
          if (category.categoryType === "income") {
            return (month.budgetedIncomeTotal += range.budgetedValue);
  ...
  budget.entries.map((entry) => {
    let matchedMonth = months.filter(
      (month) => month.label === dateToMonthStr(entry.createdAt)
    );
